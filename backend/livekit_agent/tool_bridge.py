...
class JarvisToolBridge(llm.FunctionContext):
    ...
    @llm.ai_callable(description="Fetch the latest top news headlines from BBC News.")
    ...
    @llm.ai_callable(description="Fetch recent email summaries from the Database.")
    ...
    @llm.ai_callable(description="Draft an email.")
    ...
    @llm.ai_callable(description="Send an existing email draft.")
    ...
    @llm.ai_callable(description="Fetch today's calendar events.")
    async def get_calendar_events(self) -> str:
        tool = self.lc_tools.get('get_calendar_events')
        if not tool: return "Tool not found."
        return await tool.func()

    @llm.ai_callable(description="Schedule a new calendar event.")
    ...
    @llm.ai_callable(description="Fetch tasks from the database.")
    async def list_tasks(self) -> str:
        tool = self.lc_tools.get('list_tasks')
        if not tool: return "Tool not found."
        return await tool.func()

    @llm.ai_callable(description="Create a new task.")
